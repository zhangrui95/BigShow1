...
export default class RegulaterWarnings extends PureComponent {
  ...
  enterListPage = params => {
    ...
    this.props.dispatch(
      routerRedux.push(
        `/problem/unarea/Transfer/Index?org=${areaCode}&endTime=${endTime}&startTime=${startTime}&code=${code}`
      )
    );
  };
        ...
        <Row>
          <Col span={12}>
            <div className={styles.blockTitle}>
              {configUrl.isShowBaqsstj ? '办案区数据' : '案件状态统计'}
            </div>
            ...
            <div
              className={styles.warningEcharArea}
              style={window.configUrl.isShowBaqsstj ? null : { height: 470 }}
            >
              <div id="warningEchar" style={{ height: '100%', width: '85%' }} />
            </div>
          </Col>
              ...
              renderItem={item => (
                <List.Item>
                  <List.Item.Meta
                      ...
                      configUrl.isShowBaqsstj ? (
                        <div className={styles.warningListTitle}>
                          <span className={styles.title}>
                            ●
                            <span className={styles.date} style={{ padding: '0 10px' }}>
                              {item.gjsj}
                            </span>
                            {item.name}
                          </span>
                    ...
                    description={<div className={styles.warningListContent} />}
                  />
                </List.Item>
              )}
            />
