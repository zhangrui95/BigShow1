    ...
    return (
      <div className={styles.componentBlock}>
        <div className={styles.componentBlockHeader}>
          <span
            className={nowTab === 'all' ? styles.tabHeader : null}
            style={{ width: '20%' }}
            onClick={() => this.clickTab('all', areaCode)}
          >
            全部
          </span>
          <span
            className={nowTab === 'today' ? styles.tabHeader : null}
            style={{ width: '20%' }}
            onClick={() => this.clickTab('today', areaCode)}
          >
            本日
          </span>
          <span
            className={nowTab === 'week' ? styles.tabHeader : null}
            style={{ width: '20%' }}
            onClick={() => this.clickTab('week', areaCode)}
          >
            本周
          </span>
          <span
            className={nowTab === 'month' ? styles.tabHeader : null}
            style={{ width: '20%' }}
            onClick={() => this.clickTab('month', areaCode)}
          >
            本月
          </span>
          <span
            className={nowTab === 'year' ? styles.tabHeader : null}
            style={{ width: '20%' }}
            onClick={() => this.clickTab('year', areaCode)}
          >
            本年
          </span>
        </div>
        <p id="main" style={{ width: '100%', height: '90%' }} />
      </div>
    );
  }
}
