...
export default class PoliceSituationWarningCount extends PureComponent {
  ...
  getPoliceSituationWarningCount = () => {
          let notHandleNum = 0;
          let noResultNum = 0;
          let dataList = [ { name: '入柜', count:  Math.floor(Math.random()*(100 - 1) + 1), jjly_dm: '001' },
            { name: '出柜', count:  Math.floor(Math.random()*(100 - 1) + 1), jjly_dm: '002' },]
          for (let i = 0; i < dataList.length; i++) {
            if (dataList[i].name === '入柜') notHandleNum = dataList[i].count;
            if (dataList[i].name === '出柜') noResultNum = dataList[i].count;
          }
          this.setState({
            notHandleNum,
            noResultNum,
