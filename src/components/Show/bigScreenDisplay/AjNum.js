...
export default class AjNum extends PureComponent {
  componentDidMount() {
    const { selectDate, org, orgCode, orglist } = this.props;
    this.showEchart();
    this.getAjCount(selectDate[0], selectDate[1], org, orgCode, orglist);
  ...
  showEchart = () => {
    myChart = echarts.init(document.getElementById('AjCount'));
  ...
  render() {
    return <div id="AjCount" style={{ height: '100%', width: '100%' }} />;
  }
}
