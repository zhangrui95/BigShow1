      ...
      <div>
        ...
        >
          <Row className={styles.detailRow}>
            <Col span={12}>
              扣分值：
              {this.state.detail ? this.state.detail.sum : ''}
            </Col>
            <Col span={12}>
              扣分项目：
              {this.state.detail ? this.state.detail.kfxm_mc : ''}
            </Col>
            <Col span={12}>
              被考评单位：
              {this.state.detail ? this.state.detail.bkpdw_mc : ''}
            </Col>
            <Col span={12}>
              被考评人：
              {this.state.detail && this.state.detail.bkpr_name1
                ? this.state.detail.bkpr_name1 + '、'
                : ''}
              {this.state.detail && this.state.detail.bkpr_name2
                ? this.state.detail.bkpr_name2
                : ''}
            </Col>
            <Col span={12}>
              案件名称：
              {this.state.detail ? this.state.detail.ajmc : ''}
            </Col>
            <Col span={12}>
              案件编号：
              {this.state.detail ? this.state.detail.ajbh : ''}
            </Col>
            <Col span={24}>
              扣分说明：
              {this.state.detail ? this.state.detail.kfsm : ''}
            </Col>
            <Col span={24}>
              整改结果：
              {this.state.detail && this.state.detail.zgjg
                ? this.state.detail.zgjg.replace(/\/r\/n/g, '')
                : ''}
            </Col>
          </Row>
          ...
          <Table
            size={'middle'}
            dataSource={this.state.detailList ? this.state.detailList.list : []}
            columns={columns}
